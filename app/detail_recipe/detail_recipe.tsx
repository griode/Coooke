...
export const DetailPage = ({
  ...
  return (
    <AlertDialog header={<></>} setShowDialog={setShowDialog}>
      <div className="p-0 md:p-5">
        <div className="bg-white h-screen md:h-full rounded-none md:rounded-2xl flex-col md:flex-row flex w-full md:space-x-6 overflow-y-scroll scrollbar-hidden md:overflow-hidden shadow-md">
          {/* Recipe image view */}
          <div className="w-full md:w-1/2 md:h-full bg-slate-100 rounded-none md:rounded-l-2xl overflow-hidden">
            {loadImageError || recipe.image == "" || recipe.image == null ? (
              <div
                className={
                  "text-9xl text-slate-700 w-full h-64 md:h-96 md:py-80 flex justify-center items-center"
                }
              >
              ...
              <div className="mt-4 flex-1 border p-2 rounded-xl">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {recipe.ingredients.map((ingredient, index) => (
                    <div key={index} className="bg-slate-200 p-2 text-sm rounded-lg">
                      {ingredient}
                    </div>
                  ))}
                </div>
