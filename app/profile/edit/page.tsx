...
const EditProfilePage = () => {
  ...
  const { currentUser, userPhotoUrl, setUserPhotoUrl, userInfo, setUserInfo } =
    useCurrentUser();
  ...
  return (
    <EditProfileContainer pageEditIndex={0}>
      {showSaveMessage && (
        <div className="shadow-xl flex text-sm fixed md:bottom-4 right-4 bg-slate-800 text-white px-3 py-2 rounded-full">
          <CiCircleCheck className="mr-1 text-xl" />
          Informations is save successfully
        </div>
      )}

      <form id="info-form" onSubmit={oneSaveHandler}>
        ...
        <div className="space-x-4">
          ...
          <FillButton disabled={isUpdate} className="w-fit">
            Save
          </FillButton>
        </div>
      </form>
